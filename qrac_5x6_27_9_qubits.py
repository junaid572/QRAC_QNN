...
def encode_image_tuple_qrac_3(img):
    ...
    # Split into 9 triples
    triples = [(f[i], f[i+1], f[i+2]) for i in range(0, 27, 3)]
...
#################
print("qrac_5x6_27_9_qubits.py")
datafname = 'reduced_dataset_27pixels_QRAC.npz'
...
# export history
fname = f"classify_5x6_QRAC_Coding_DataQRACOptimized_bs{batch_size}_Ns{N_samples}_Nh{N_shot}_Nl{num_layers}.dat"
export_history(history, fname)
fname = f"classify_5x6_QRAC_Coding_DataQRACOptimized_bs{batch_size}_Ns{N_samples}_Nh{N_shot}_Nl{num_layers}.h5"
