...
def encode_image(img):
    # Elementary encoding of a 4x3 binary image into a quantum circuit
    qubits = [cirq.GridQubit(0, i) for i in range(12)]
    circuit = cirq.Circuit()
...
print("elementary_coding_4x3_12_qubits.py")
datafname = 'reduced_dataset_12pixels_V2.npz'
...
# 4 x 3 image with block averaging to get 12 features
# X_features = np.array([extract_12_features(img) for img in X_filtered])
...
fname = f"classify_4x3_elementary_Coding_DataV2_bs{batch_size}_Ns{N_samples}_Nh{N_shot}_Nl{num_layers}_dD{del_duplicates}.dat"

export_history(history, fname)

fname = f"classify_4x3_elementary_Coding_DataV2_bs{batch_size}_Ns{N_samples}_Nh{N_shot}_Nl{num_layers}_dD{del_duplicates}.h5"
